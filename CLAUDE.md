...
- ✅ **[F-PLAYER] MERGÉ le 1er août** (https://trello.com/c/AZ3U4BZl, branche de William, signée à son identité `Omshinwa <fynmorph@gmail.com>`) : `/players/$pseudo` sert le profil, ses classements avec **rang et taille du ladder**, ses équipes, et les actions de relation. Récit, review et correctifs → `docs/frontend.md` + `docs/journal.md`. Ne restent ici que trois règles :
  ...
  - ✅ **`player.mjs` existe (9 checks)**, écrit avec le lot de correctifs du 1er août : fiche d'un inconnu, demande d'ami, **changement de joueur sans rechargement**, blocage d'un ami, écran de fin, mise à jour du rail, 404 du profil bloqué. Les 4 défauts ont été **remis dans le code et vus ROUGES**. ⚠️ **La campagne complète n'a pas été relancée depuis** (seuls `player`, `teams-detail` et `solo` l'ont été).
  - 🚨 **UNE ROUTE À PARAMÈTRE N'EST PAS REMONTÉE QUAND LE PARAMÈTRE CHANGE** — l'état local survit d'un joueur au suivant. Vécu, et pas qu'une fois : la fiche joueur est montée sur `key={pseudo}` pour ça. **Tout écran `/x/$id` porteur d'état local a le même piège.** → `docs/pieges.md`
  - 🚨 **NE JAMAIS RÉGLER UN BESOIN LOCAL EN MODIFIANT UN COMPOSANT PARTAGÉ.** [F-PLAYER] avait retiré `text-xs` d'`ui/avatar.tsx` pour un seul appelant (**20 autres** en dépendaient) et **dupliqué `ui/stat-strip.tsx`** en migrant `TeamHero` sur la copie. Annulé. **Un composant `ui/` se plie, il ne se réécrit pas.** 🔑 **Le besoin, lui, était RÉEL** (initiales de 12 px dans un cercle de 80) : réglé en rendant la taille **surchargeable** — `text-xs` posé sur le conteneur d'`Avatar`, où `cn`/tailwind-merge laisse l'appelant gagner, défaut inchangé pour les 20 autres (`2d61772`). ⚠️ **`TeamHero` et `SoloHero` gardent le même défaut visuel** : même cercle, mêmes 12 px — une classe à ajouter le jour où on le décide.
  - ⚠️ **CE QUI RESTE À FAIRE SUR CETTE PAGE** : ① relancer la **campagne complète** (on a touché `Avatar`, monté par 21 écrans, et `TeamHero`) ; ② la **bande du hero** fait ~130 px de dégradé quasi noir et vide — un joueur n'appartient à aucun jeu, donc pas d'artwork possible, **choix produit à trancher** (la réduire, ou y mettre une donnée vraie) ; ③ le scénario n'exerce **pas** « Refuser », « Annuler ma demande » ni « Retirer des amis ».
