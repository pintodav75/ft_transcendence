...
- **Tests** : dernière campagne complète **3 août — 28 scénarios, 467/468 checks, console 0 partout**. Vitest 40/40, build de prod OK **dans le conteneur** (sur l'hôte il sort EACCES : `dist` appartient à root, artefact local, pas un défaut). `exit 0` **est** le critère (1 = check rouge, 2 = harnais en échec). → **`backend/tests/README.md`** et `frontend/tests/console-audit/README.md`
  - 🚨 **UN SEUL ROUGE CONNU, ET IL EST IMPUTÉ AU HARNAIS, PAS À L'APP** : `fs1-friends` check `F4` (fermeture du menu d'actions par Échap). **Vert seul, vert en binôme, vert en trio ; rouge uniquement quand les 5 premiers scénarios tournent avant** — et il entraîne un crash en §5, donc la campagne complète sort en **`exit 2`**. Cause **non identifiée** (admin résiduel, notification en attente et ami résiduel vérifiés en base, tous à zéro ; l'hypothèse « course de rendu » est **écartée** — une attente franche de 5 s ne la corrige pas). 🔑 **Ne pas « corriger » le composant sur la foi de ce rouge** — le détail vit en commentaire dans le scénario.
...
- 🎯 **POINT DE REPRISE — 3 août, après `dbc1692`.** 🚨 **LE DÉVELOPPEMENT EST TERMINÉ.** Plus aucune carte en Todo, In Progress ou Review ; plus aucune branche de coéquipier hors de master ; le board est vide. **18 points sur un plafond de 19.** **Il ne reste QUE le README et la préparation de la soutenance.** **Dans l'ordre :**
  1. ✅ **[F4B]** (2 août) et ✅ **[FIX-LANDING-RAIL]** (3 août, `dbc1692`, branche de William) sont mergées. Récits → `docs/frontend.md` et `docs/journal.md`.
  ...
  4. **Réglages de livraison** : `RATE_LIMIT_FACTOR` est à **1000** dans le `.env` de David (doit valoir **1**) · **0 compte admin en base** (sans admin, l'arbitrage n'est pas démontrable) · **tri de `/search`** : les résultats **sont** triés (alphabétique, global aux deux types), mais l'utilisateur ne peut pas **choisir** le tri. 🚨 **Ne pas le revendiquer à l'oral tant que ce n'est pas exposé** — `docs/modules.md` disait « tri » et a été corrigé le 3 août. L'exposer reste ~10 lignes et sécurise 1 pt, c'est optionnel.
  ...
  - 🚨 **NETTOYAGE « traces de Claude » — décision de David, à faire avant de rendre.** Trois faits mesurés le 3 août : ⓵ supprimer `CLAUDE.md` et `docs/` **ne les efface pas de l'historique** (125 commits touchent `CLAUDE.md`) — **ne PAS réécrire l'historique** juste avant la soutenance ; ⓶ **3 commits portent `Co-Authored-By: Claude`**, tous sur master, signés `pintodav75` (commit initial, OAuth Google, 2FA) ; ⓷ 🔑 **le sujet EXIGE de déclarer l'usage de l'IA dans le README** — supprimer les fichiers de travail est légitime, mais le README doit dire que l'IA a été utilisée, sinon la suppression ressemble à une dissimulation que l'historique contredit.
