...
- **Tests** : `npm run audit` (audit console Chrome) sort **0 sans filtre** depuis FT-2A — dernière campagne complète **28 scénarios / 467 checks / 0 rouge** (2 août, après `[F4B]`). `exit 0` **est** le critère (1 = un check rouge, 2 = harnais en échec). Vitest 40/40 (helpers purs) + **22 suites e2e Python** sur la vraie base de dev. → **`backend/tests/README.md`** et `frontend/tests/console-audit/README.md`
...
- 🎯 **POINT DE REPRISE — 2 août, après le merge `4de7683` ([F4]).** **Le produit est FINI et VERT** : campagne **28 scénarios / 467 checks / 0 rouge**, console 0, responsive validé de **320 à 768 px**, WCAG 2.1 AA conforme, `/profile` compris. **18 points sur un plafond de 19.** **Aucun chantier en cours**, **aucun inconnu technique** dans ce qui reste, et 🚨 **PLUS AUCUNE BRANCHE DE COÉQUIPIER HORS DE MASTER**. **Ce qui reste, dans l'ordre :**
  1. ✅ **[F4B] TERMINÉE** (2 août) — liaison de compte externe + suppression de compte. **Plus aucun développement produit en attente.** Récit → `docs/frontend.md` et `docs/journal.md`.
