...
│       │   └── ladders.ts    # read-only (list + detail)
...
- ⚠️ **Aucun script de seed** : les tables `games`/`ladders` sont vides tant qu'on n'a pas inséré les données du §3.2/§3.4 du design. Les read routes renvoient donc du vide en l'état.
