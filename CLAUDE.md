...
- 🎯 **CHANTIER EN COURS : LE RAIL SOCIAL.** 🚨 **Walid (`wacista`) travaille avec nous sur place : TOUT le rail social se commit, se pousse et se merge à SON identité** — `git -c user.name=wacista -c user.email=wacista@student.42.fr …`. Ne jamais signer une de ces branches autrement. Ordre arrêté le 31/07 : **`FS-0 → FS-1 → FS-3 → FS-4 → FS-2 → FS-5`**, 6 cartes, **aucun ticket backend** (B-SOC couvre tout). Carte parapluie https://trello.com/c/NBGs3xNc. Charger la mémoire `project_f_social_decomposition` avant d'y toucher. Récit du rebase et des pièges → `docs/journal.md` + `frontend/tests/console-audit/README.md`.
  - ✅ **LE RAIL SOCIAL EST TERMINÉ** — les 6 cartes sont mergées. Campagne **26 scénarios / 399 checks / exit 0**, console 0. → `docs/frontend.md`
  - 🚨 **BLOQUER SUPPRIME L'AMITIÉ, des deux côtés** (mesuré le 01/08) : plus de conversation, plus d'historique, plus de recherche, et la page du joueur rend **404 aux DEUX** — y compris à celui qui a bloqué. On ne débloque donc que depuis la liste « Blocked players » de l'onglet Ajouter. 🔑 **On ne dit JAMAIS à quelqu'un qu'il est bloqué** : le serveur répond « introuvable », le même message que pour un compte inexistant. ⚠️ Le code d'erreur `blocked` du chat est **inatteignable** (l'amitié ayant disparu, c'est `not_friends` qui part) — code défensif, pas un bug.
  - 🚨 **IL Y A 17 TYPES DE NOTIFICATIONS, PAS 16.** `match_cancelled_member_left` (migration `0024`, BX-LEAVE) **manquait au miroir de validation du front** : ces notifications étaient **rejetées en silence** et n'arrivaient jamais en direct. Trouvé pendant FS-2. 🔑 **Toute valeur ajoutée à l'enum côté serveur doit l'être aussi dans `frontend/src/lib/realtime-schema.ts`** — rien ne le signale, l'exception est avalée.
  - ⚠️ **Une notification raconte un FAIT PASSÉ : le droit qu'on avait à l'envoi peut être perdu au clic.** Deux liens ont dû être retirés pour ça (`match_cancelled_member_left` → renvoie au ladder ; `dispute_needs_admin` → aucun lien, la lecture du dossier revérifie `is_admin` **à chaque appel** et l'équipe le retire avant chaque campagne). **Ne jamais lier vers `/teams/$id` ni `/players/$pseudo` depuis une notification** : l'équipe peut être dissoute et le compte supprimé.
  - 🔑 **`AuthenticatedLayout` monte DEUX `SocialPanel`** (rail permanent + overlay mobile) : tout compteur ajouté au rail doit s'en garder, sinon il compte double.
  - 🚨 **LES TROIS PANNEAUX DU RAIL RESTENT MONTÉS** (les inactifs masqués), depuis FS-3 : sans ça un aller-retour d'onglet reprenait le brouillon de chat à zéro. Trois conséquences — un panneau **masqué n'annonce rien**, **changer d'onglet ne recharge plus rien** (un scénario qui croit provoquer un rechargement ainsi est vert par construction), et **la garde « ne rien charger » ne peut pas être « le composant est monté »** : c'est la visibilité de l'onglet qui décide.
  - 🚨 **LE RAIL CHARGE SA DONNÉE SUR TOUTES LES PAGES AUTHENTIFIÉES.** `home.mjs` exclut ces appels de son budget de requêtes via une liste **`SOCIAL_RAIL`** — **y ajouter une ligne à chaque carte du rail, plutôt que de remonter le budget** (sinon on croit trois fois de suite à une régression de `/home`). Tout ce qui n'y est pas reste imputé à la page, `GET /ladders` compris : c'est ce qui maintient `N5` de `f-nav` en vie.
  ...
  - 🚨 **PAS DE FENÊTRE FLOTTANTE SOUS 1024 px** (FS-4) : le panneau mobile est une modale faite main dont l'`Escape` vit sur `document`, empiler une fenêtre par-dessus rejoue ce piège. La garde est un vrai `matchMedia`, **pas** un masquage CSS — masquer laisserait les fenêtres montées.
  - 🔑 **`GET /friends` rend DEUX identifiants et ils ne sont pas interchangeables** : `id` = l'ami (bloquer, ouvrir son profil, lui écrire), `friendshipId` = la relation, **seul** identifiant accepté par `DELETE /friends/{id}`.
  - 🔑 **QUATRE FAUX VERTS TROUVÉS EN GARDANT DES DÉFAUTS DE REVIEW**, dont deux sur ce rail. La seule méthode qui les débusque : **remettre le défaut dans le code et regarder le check rougir**. Un check écrit après coup est faux vert jusqu'à preuve du contraire. → `frontend/tests/console-audit/README.md`
