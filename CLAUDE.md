...
- 🎯 **2 modules restent gagnables SANS CODE, par le seul README** : *Custom design system* (`components/ui/` en compte **25**, le sujet en demande 10) et **Modules of choice / Major** pour le cycle compétitif (challenge/accept + Elo + disputes + jobs 24 h), qu'**aucun** module de la liste ne couvre.
...
- **Frontend** : **toutes les pages du rail sont remplies** — `/home`, `/teams`, `/teams/$teamId`, `/solo`, `/solo/$ladderId`, `/games`, `/games/$gameId`, `/ladders/$ladderId`, `/matches/$matchId`, `/matchmaking`, `/history`, `/disputes/$disputeId`, `/admin/disputes` (admin). **Plus aucune page vierge, plus aucun cul-de-sac.** 🔑 **Règle produit : tout nom d'équipe et tout joueur est cliquable vers sa page** — sauf compte supprimé, équipe dissoute, ou ligne déjà entièrement cliquable (pas de lien dans un lien). Le cycle challenge/accept est bouclable **de bout en bout à la souris** depuis F-MM. `/privacy` et `/terms` sont remplies ([FT]). **Reste à remplir** : `/profile` (F4, Adrien) — encore le stub `a faire`, seule route sans `<h1>`. → **`docs/frontend.md`**
- **Tests** : `npm run audit` (audit console Chrome) sort **0 sans filtre** depuis FT-2A — dernière campagne complète **27 scénarios / 416 checks / exit 0** (1er août, après `[A11Y-AA]`). `exit 0` **est** le critère (1 = un check rouge, 2 = harnais en échec). Vitest 40/40 (helpers purs) + **22 suites e2e Python** sur la vraie base de dev. → **`backend/tests/README.md`** et `frontend/tests/console-audit/README.md`

📄 Historique daté des merges, décisions et pièges de chaque ticket → **`docs/journal.md`**

### Prochaines actions

- 🔍 **AUDIT COMPLET CONTRE LE SUJET v21.1 — 1er août.** PDF lu **en entier** ; il a produit le décompte de modules ci-dessus, les 4 correctifs d'accessibilité et la liste du point de reprise. 📌 **Décision de David : README plus tard**, quand `/profile` sera là et les commits mieux répartis — ⚠️ **Adrien et William ont travaillé avec lui, mais David a tout committé** : le déséquilibre du `git log` est un artefact, pas la réalité des contributions. Détail → mémoires `project-audit-modules-v21`, `project-audit-gaps-obligatoires`, `project-reste-a-faire-soutenance`.
  - ✅ **RESPONSIVE + WCAG AA — mergés le 1er août** (`b8b75fc`, merge `2ec3210`). Deux règles qui mordent quiconque touche au shell ou aux tokens : 🔑 **`components/layout/PrimaryNav.tsx` est la SEULE maison des 6 items de nav**, servie par le rail (≥ `lg`) **et** par le tiroir mobile — les deux ne coexistent jamais dans l'arbre d'accessibilité ; 🔑 **`--color-border-control` ne sert QU'AUX CONTRÔLES** (input/select/textarea/bouton secondaire/Google) : la bordure d'un champ doit tenir 3:1, un séparateur décoratif en est **exempté**, et tout repasser en 3:1 ferait une grille de cases. Récit, mesures et justifications → `docs/frontend.md` (`[FIX-RESPONSIVE]`, `[A11Y-AA]`)

- 🎯 **POINT DE REPRISE — 1er août, après le merge `2ec3210`.** **Le produit est FINI et VERT** : campagne **27 scénarios / 416 checks / exit 0**, console 0, responsive validé de **320 à 768 px**, WCAG 2.1 AA conforme. **18 points sur un plafond de 19.** **Aucun chantier en cours**, et **aucun inconnu technique** dans ce qui reste. La seule branche de coéquipier hors de master est **[F4]** (Adrien). **Ce qui reste, dans l'ordre :**
  1. **F4 puis F4B d'Adrien** — le seul vrai développement. 🚨 **F4B (liaison de compte externe + suppression)** est l'enjeu : sans elle, un joueur ne peut pas lier son Steam, donc **ne peut pas utiliser la fonction centrale du site**.
  2. 🚨 **Que chacun sache EXPLIQUER son code** — le sujet y consacre son **chapitre I** et peut demander une **modification en direct** ; avec Adrien à 2 commits et William à 13 sur 260, redistribuer les commits **ne suffit pas**. Risque n°1, devant le README.
  3. **README** (anglais, sections exigées) → enlève le motif de rejet n°1 **et** embarque les 2 modules gratuits (18 → 21 pts).
  4. **Réglages de livraison** : `RATE_LIMIT_FACTOR` est à **1000** dans le `.env` de David (doit valoir **1**) · **0 compte admin en base** (sans admin, l'arbitrage n'est pas démontrable) · **tri à exposer sur `/search`** (~10 lignes, sécurise 1 pt).
  5. **Répéter la soutenance** — **12 modules** à démontrer en direct.
  - ⚠️ **Quand F4 arrive** : `/profile` est le seul écran **jamais passé** aux audits responsive et accessibilité (et la seule route sans `<h1>`) — **le repasser dans les deux, sinon il fait tomber le module WCAG AA**.
