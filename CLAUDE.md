...
- 🎯 **PROCHAIN TICKET : [FT-4A]** (https://trello.com/c/yilRzYrQ) — page match en **lecture seule**. **[FT-4] a été DÉCOUPÉ en deux le 29/07**, décision argumentée avec 3 mesures : ① la page équipe, de complexité comparable (438 lignes), avait pris **3 tickets** (FT-2A/2B/2C) ; ② le scénario d'audit de l'écriture serait le plus cher jamais écrit (deux comptes, soumission puis confirmation, vérification de l'Elo) alors que le plus gros existant, `teams-manage.mjs`, fait **816 lignes** pour moins ; ③ le helper SQL demandé est une **capacité neuve** — le runner d'audit ne fait aujourd'hui **aucun** appel système, tout passe par HTTP. La couture tombe entre **lecture pure (4A)** et **écriture pure (4B)**, donc rien n'est coupé au milieu, et 4A a de la valeur seul (le placeholder disparaît, les 6 états semés deviennent visibles). **[FT-4B]** (https://trello.com/c/3mjx0M31) est en Todo. 🔑 **Décision 1v1 (29/07)** : la variante 1v1 **reste dans 4A** et le scénario d'écriture de 4B tourne sur un ladder **1v1** — un match 1v1 demande **2** comptes jetables avec 1 compte externe lié chacun, contre **10 + 10** pour deux lineups 5v5. ⚠️ Le back gère complètement le 1v1 (`sideTeamId = null`, le joueur **est** le camp, pas de lineup) mais **aucun match 1v1 n'existe en base et aucun ne peut être créé depuis l'interface** tant que `/solo` est un placeholder → **4A ajoute au seed un match 1v1 chess**, sinon la variante se développe à l'aveugle.
