...
- 🎯 **POINT DE REPRISE — 3 août, après `dbc1692`.** 🚨 **LE DÉVELOPPEMENT EST TERMINÉ.** Plus aucune carte en Todo, In Progress ou Review ; plus aucune branche de coéquipier hors de master ; le board est vide. **21 points revendiqués depuis le README, plafond 19.** **Il ne reste QUE le README et la préparation de la soutenance.** **Dans l'ordre :**
  ...
  3. ✅ **README RÉDIGÉ le 4 août** (anglais, 12 sections, 21 pts listés / plafond 19). ⚠️ **Reste la section « Individual contributions » : 4 blocs vides à faire remplir avant la soutenance** — une section vide = README incomplet = motif de rejet. Enlève le motif de rejet n°1 **et** embarque les 2 modules gratuits (18 → 21 pts). 🚨 **Y mettre `seed:demo` + le tableau des comptes**, jamais `seed:dev`/`seed:social` (interne). 🚨 **Ne rien écrire sur les tests** : revendiquer des tests qu'on ne peut pas montrer est pire que le silence. 🚨 **Le bonus n'est évalué QUE si la partie obligatoire est parfaite** — un README incomplet n'enlève pas qu'une case, il annule les 4 points au-dessus de 14.
  4. **Réglages de livraison** : `RATE_LIMIT_FACTOR` est à **1000** dans le `.env` de David (doit valoir **1**) · ✅ **rôles attribués (3 août, décision de David)** : **David = PO**, **Adrien = PM**, **Walid = Tech Lead**, **William = Developer** — tous les 4 sont Developer en plus (le sujet §II.1.1 l'impose). ⚠️ **Walid doit savoir défendre les 5 choix de stack à l'oral** (Fastify, PostgreSQL, Drizzle, WS natif, origine unique sans Nginx) : le correcteur interroge le Tech Lead là-dessus. À écrire dans le README, section « Team Information » (rôle **+ responsabilités**) · **tri de `/search`** : les résultats **sont** triés (alphabétique, global aux deux types), mais l'utilisateur ne peut pas **choisir** le tri. 🚨 **Ne pas le revendiquer à l'oral tant que ce n'est pas exposé** — `docs/modules.md` disait « tri » et a été corrigé le 3 août. L'exposer reste ~10 lignes et sécurise 1 pt, c'est optionnel.
