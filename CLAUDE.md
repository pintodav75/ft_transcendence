...
- **Reste FL** : garde de route (ticket séparé), barre de nav mobile + overlay tactile, compteur de joueurs (le front accepte déjà `playerCount?: number` mais affiche `—` : le back est à faire → **carte [B10]**, https://trello.com/c/7UdiDepz. ⚠️ La piste `user_external_accounts` notée ici à l'origine a été **écartée** en concevant B10 : `riot` sert à val **ET** lol, `steam` à cs2 → deux jeux, même chiffre. La définition retenue est l'union `team_members` ∪ `match_participants` par ladder du jeu), `gameHref` (table vide) + `<Link>`, i18n, bascule sur la codegen OpenAPI.
...
**Frontend — F0-B, Fast Refresh, FR1 Register, F0-D, F-Nav, FR2 Login + 2FA et FL Landing publique mergés.**
