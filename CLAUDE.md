...
- ✅ **Tests — deux niveaux, deux outils** (`backend/tests/`) :
  ...
  - **End-to-end (Python, stdlib seule)** — `cd backend/tests && python3 run_all.py` : **13 suites, 463 cas** (mesuré 24/07 : **461 ✅ / 2 ❌** — les 2 rouges sont un **défaut du test, pas du produit** : `test_notifications.py:305` et `:321` comptent **toutes** les notifs de `dave1` au lieu de filtrer par type+match comme leurs voisines, or `dave1` reçoit légitimement un `team_member_added` depuis **B11**. À corriger dans un ticket dédié) (+ cas jobs via `B6_JOBS=1` / `B7_JOBS=1` / `B9_JOBS=1`) qui tapent sur le **vrai** backend et la **vraie** base de dev, sans mocks. Les users de test sont créés puis **supprimés** (motif `^(alice|bob|carol|dave|erin)[0-9a-f]{8}$`) → les données de l'équipe ne sont jamais touchées. `helpers.py` gère le rate-limit de `register` (3/min) et l'accès SQL direct (pour forcer des états que l'API ne permet pas encore d'atteindre). Voir `backend/tests/README.md`.
