...
- 🔴 **[FT-3] EN COURS sur `feature/ft3-ladder-page` — code livré, review passée, NON MERGEABLE en l'état.** ⚠️ Le code a été produit **depuis un chat mobile, sans `coder-front`** ; il est **non committé** (arbre de travail, `git log master..HEAD` est vide) — les commits de la branche sont `9f5cdec` (`RATE_LIMIT_FACTOR`, invariant #13) et les corrections ci-dessous, **hors périmètre FT-3** pour le premier. Rapport `reviewer-front` (Opus) du 28/07, **3 bloquants — TOUS TRAITÉS le 28/07** :
  1. ✅ **CORRIGÉ (28/07).** **Règles incomplètes** — `LadderRules.tsx` omet 3 des 5 règles demandées : créneau au **quart d'heure + 15 min de préavis** (`SLOT_GRID_MINUTES`/`MIN_LEAD_MINUTES`, `routes/matches.ts:33,37`), **litige ouvert automatiquement** sur désaccord (`matches.ts:1183-1207`), et le sort d'un slot non accepté. ⚠️ **La carte Trello ment sur ce dernier point** : elle dit « annulé au bout de 24 h », le code (`jobs/index.ts:37-46`) annule tout `pending` dès qu'il est à **moins de 15 min** du coup d'envoi. Afficher la règle du CODE. → Les 3 règles (`Slot`, `Disagreement`, `Unclaimed slot`) sont dans `LadderRules.tsx`, valeurs **importées** de `lib/team-detail` (`SLOT_GRID_MINUTES`, `MIN_LEAD_MINUTES`, `MAX_OPEN_SLOTS`), jamais recopiées ; checks d'audit **L7b** (les 5 libellés exigés) et **L7c** (`lockoutMinutes` = valeur de l'API) posés pour verrouiller.
  2. ✅ **CORRIGÉ (28/07) — et ce n'était PAS FT-3.** **`npm run audit` sortait 1** (2 rouges, rejoué 2×, aucun dans le diff FT-3) : `ft1c-team-logo` 4.1b rouge **même en isolation** — `scenarios/ft1c-team-logo.mjs:107-108` fait `[role=status]`.`first().waitFor()`, le piège documenté (région live montée en permanence → attendre le **texte**) ; `teams-manage` B13c-bis rouge en campagne, **vert seul** (suspect : la disparition des pauses de quota, cf. invariant #12 — **tranché** : 2 **courses latentes du harnais**, pas une régression — voir invariant #11 et le helper `awaitAnnouncement`. **Campagne complète rejouée : 10 scénarios, 124/124, exit 0.**)
  3. ✅ **ARBITRÉ PAR DAVID (28/07) : suppression VALIDÉE**, l'entrée vers les ladders passera par **`/games`** (pas encore codé → carte à créer). **Suppression de `/ranking`** (page + route + `RankingTable` + entrée de nav) alors que la carte demandait de lui faire **consommer** la ligne extraite. Techniquement propre (0 lien mort), mais **plus aucun point d'entrée vers un classement** : `/games` est un stub, `play`/`find party` sont des `MenuItem` sans `to` → un compte **sans équipe** n'atteint plus aucun ladder. **Arbitrage de David requis** : valider + ouvrir une carte « entrée de nav vers les ladders », ou restaurer la page en consommant `LadderBoard`.
  - **Ce qui est bon et se garde** : extraction réelle de `LadderRow`/`LadderBoard` dans `components/ladders/` (aucune classe Tailwind recopiée, `LadderExcerpt` consomme bien le composant extrait), `GET /ladders/{id}` enrichi de `maps` + `openapi.yaml` + `api-types.gen.ts` **byte-identique** à une régénération fraîche, états 404/vide propres, lint 0/0, `tsc -b` 0, build depuis un état propre en 316 ms, `colW=601 / overflow=0` mesuré à 1280.
  - **NON-BLOQUANTS : 8 corrigés le 28/07, 1 refusé.** Corrigés : artwork du jeu (`GameImage` réutilisé), ligne de sa propre équipe surlignée (`useMyTeams` + `find` sur `ladderId` — un joueur a AU PLUS une équipe par ladder), pluriel « 1 map », prose de `openapi.yaml` sur `lockoutMinutes` (+ `api-types.gen.ts` régénéré : description seule, aucun type modifié), docs `frontend.md`/`stack.md` purgées de `/ranking`, **check de largeur rendu honnête** (L9b mesure la boîte ET la largeur rendue du nom — l'ancien L9 regardait `documentElement`, que l'`overflow-hidden` de `LadderBoard` masquait, il était vert par construction), **nom du compétiteur lisible à 375 px** (les tracks fixes + gaps + padding mangeaient 268 px d'une boîte de 276, l'avatar étant `shrink-0` le nom était rendu **0 px** et 73 px sortaient en silence → sous `sm` les 3 nombres passent sous le nom via `sm:contents`, au-dessus la grille à 5 colonnes est restaurée à l'identique), et **lignes de classement enfin auditées** (interception `page.route`, checks L11→L14 : liens `user`/`team`, surlignage non-lien, `aria-label`, focus clavier). 🚫 **REFUSÉ — pagination/défilement (décision de David, 28/07, mesure sous les yeux)** : à 200 compétiteurs la page fait **11 431 px (16 écrans)** pour 2941 nœuds DOM et 1278 ms de rendu. Le coût DOM n'est pas le sujet ; on **accepte** que l'utilisateur descende la page. En échange le Ctrl+F du navigateur porte sur tout le classement et personne n'atterrit sur une « page 1 » qui ne le contient pas. **Ne pas re-ticketer sans qu'un ladder dépasse la centaine.** Rationale figée dans le docblock de `LadderBoard.tsx`.
  - **Non-bloquants d'origine, pour mémoire** : pas d'artwork du jeu (`components/games/GameImage.tsx` existe), pas de pagination/défilement, ligne de sa propre équipe non surlignée (`LadderBoard` appelé sans `selfTeamId`), **à 375 px le nom du compétiteur fait 0 px** (piste 6 px, 73 px clippés — dette héritée de `LadderExcerpt` mais qui passe d'un extrait de 5 lignes à une page dédiée), le check L9 du scénario mesure `documentElement.scrollWidth` alors que le débordement est clippé (vert structurellement acquis → mesurer `board.scrollWidth - board.clientWidth` comme `teams-matchmaking` M11), le scénario ne monte **jamais** de ligne de classement (base de dev vide → liens/`aria-label`/clavier non audités ; une interception `page.route` le rendrait reproductible), `openapi.yaml:368-373` décrit faussement `lockoutMinutes` comme le délai avant soumission de score (la vraie garde est `scheduledAt`, `matches.ts:1056-1058`), pluriel « 1 maps » dans `LadderMapPool.tsx:26`, et `docs/frontend.md` (§75/85/95) + `docs/stack.md` décrivent encore `/ranking` comme livrée.
