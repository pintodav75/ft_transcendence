...
## 🧩 Modules — **21 points revendiqués depuis le README du 4 août** (seuil 14, **plafond de notation 19**)

7 majors + 7 minors, **vérifiés contre le PDF v21.1** : frameworks front+back, user management, WebSocket temps réel, user interaction, **organization system** (= nos teams), **accessibilité WCAG 2.1 AA** | ORM Drizzle, OAuth 2.0, 2FA TOTP, file upload, notifications, **advanced search** | et les **2 gagnés par le seul README (4 août)** : **custom design system** (Minor, 26 composants) + **module of choice : le cycle compétitif** (Major, 4 justifications rédigées).
...
- ✅ **MASTER A BOUGÉ SANS DAVID (6 → 15 août), TOUT EST RÉCUPÉRÉ** (pull du 16/08, sans conflit). Walid a réduit la zone cliquable des liens de profil du rail ; **William a simplifié la `SearchBar`, ajouté recherche d'adversaire + tri + pagination sur `/history` (front ET back, `order=asc|desc`), retiré les devtools du routeur, puis dégraissé les commentaires de TOUT le front** (`simplify-match`, 208 fichiers, −7 469 lignes dont 6 559 de commentaires ; **908 lignes de code réel** modifiées, build et lint verts, invariants vérifiés un par un). ⚠️ **Il écrit lui-même n'avoir RIEN repassé à l'écran ni à la console** — les 4 pages touchées (matchmaking, accueil, fiche joueur, dossier de litige) sont à recliquer à la main avant la soutenance. ⚠️ Le haut de `pages/matchmaking.tsx` porte 14 lignes de **notes de brouillon** qui ressemblent à une liste de choses non faites, alors que tout est implémenté — à nettoyer.

- 🎯 **POINT DE REPRISE — 3 août, après `dbc1692`.** 🚨 **LE DÉVELOPPEMENT EST TERMINÉ.** Plus aucune carte en Todo, In Progress ou Review ; plus aucune branche de coéquipier hors de master ; le board est vide. **21 points revendiqués depuis le README, plafond 19.** **Il ne reste QUE le README et la préparation de la soutenance.** **Dans l'ordre :**
  ...
  3. ✅ **README RÉDIGÉ le 4 août** (anglais, 12 sections, 21 pts listés / plafond 19). ✅ **Blocs « Individual contributions » remplis : David le 13 août (`fcd02a7`), William le 11 (`657bea0`), Adrien le 15 (`17578dd`)** — gabarit : ~15 lignes, 3 sous-parties, ton direct, **rien sur les tests**. 🚨 **NE RESTE QUE LE BLOC DE WALID** — une section vide = README incomplet = motif de rejet. Enlève le motif de rejet n°1 **et** embarque les 2 modules gratuits (18 → 21 pts). 🚨 **Y mettre `seed:demo` + le tableau des comptes**, jamais `seed:dev`/`seed:social` (interne). 🚨 **Ne rien écrire sur les tests** : revendiquer des tests qu'on ne peut pas montrer est pire que le silence. 🚨 **Le bonus n'est évalué QUE si la partie obligatoire est parfaite** — un README incomplet n'enlève pas qu'une case, il annule les 4 points au-dessus de 14.
  4. **Réglages de livraison** : ✅ **rôles attribués (3 août, décision de David)** : **David = PO**, **Adrien = PM**, **Walid = Tech Lead**, **William = Developer** — tous les 4 sont Developer en plus (le sujet §II.1.1 l'impose). ⚠️ **Walid doit savoir défendre les 5 choix de stack à l'oral** (Fastify, PostgreSQL, Drizzle, WS natif, origine unique sans Nginx) : le correcteur interroge le Tech Lead là-dessus. À écrire dans le README, section « Team Information » (rôle **+ responsabilités**) · **tri de `/search` (mis à jour le 13 août)** : le **back accepte désormais `order=asc|desc`** (les 2 `ORDER BY` sont écrits en toutes lettres, jamais interpolés — une direction de tri ne peut pas être un paramètre lié), mais **AUCUN écran ne l'expose** : la barre du rail n'a pas de bouton de tri. 🚨 **« Advanced search » se démontre donc sur `/history`** (recherche d'adversaire + sens du tri **choisi** + pagination, livrés par William le 10 août), **pas sur la barre du rail**.
  5. **Répéter la soutenance** — **14 modules** à démontrer en direct, **plus le déploiement en une commande** : la grille §2.2 demande à un membre de le faire **devant le correcteur** (`docker compose up -d --build` suffit, pas de Makefile à écrire). ⚠️ La console Chrome est la case §2.3, **vérifiée par le correcteur lui-même** — rien à démontrer, et la grille tolère un warning de lib tierce s'il est expliqué.
  6. 🚨 **DÉCISIONS VERROUILLÉES DU 4 AOÛT, ne pas les rouvrir** : `CLAUDE.md` et `docs/` **restent livrés** (leur retrait a été proposé et refusé) · la **déclaration d'usage de l'IA du README est définitive** — cadrage, **relecture de code par un reviewer IA en plus de la review humaine**, débogage, documentation, **et rien sur la génération de code** (décision de David, posée 3 fois). ⚠️ Conséquence assumée : `docs/journal.md` garde 3 phrases qui disent qu'un agent a codé. **Les tests ne reviennent pas dans le dépôt** — le harnais front a été envoyé à Walid en `.zip` le 4 août pour qu'il valide un fix, hors git.
...
- ✅ **[F4] MERGÉE le 2 août** — `/profile` (Adrien, commit `37534e1`, merge `4de7683`). ⚠️ **Commit amendé ET merge signés à SON identité** (`acattet`), comme le rail social l'est à celle de `wacista` : le correctif d'un reviewer ne déplace pas la paternité du ticket. Récit, décisions et mesures → `docs/frontend.md` + `docs/journal.md`. Ne restent ici que les règles qui mordent quelqu'un qui n'y travaille PAS :
  ...
  - 🔑 **UN FAUX POSITIF D'`axe` SE PROUVE, IL NE SE SUPPOSE PAS** : la même sonde relancée sur une autre route dit si le défaut vient de la page ou du décor. Sans ce contrôle croisé on corrige un défaut inexistant. → `docs/pieges.md`. ✅ **La cause de ce cas-là (devtools TanStack Router) a été RETIRÉE du code le 10 août** — le double `contentinfo` ne peut plus apparaître, en dev comme en prod. ⚠️ `axe-core` **n'est pas installé dans le dépôt** — la sonde vit dans le scratchpad.
...
13. **`RATE_LIMIT_FACTOR` multiplie TOUS les quotas de l'API** (défaut 1, commit `9f5cdec`) — confort du harnais, **pas** un interrupteur off (la mécanique reste démontrable en soutenance). Toute route qui déclare un quota **doit** passer par `rlMax()` (`backend/src/utils/rate-limit.ts`). ⚠️ **DOIT valoir 1 à la livraison** : `.env.example` est à 1, compose retombe sur 1 si la variable est absente, et le backend écrit un WARN à chaque démarrage tant que ce n'est pas le cas (`console.warn`, **pas** `server.log.warn` — le serveur est instancié sans `logger`, `server.log` est muet). 🚨 **Ne jamais signaler la valeur du `.env` local de David** : elle restera à 1000, ce n'est pas une tâche restante (décision du 16 août) — la livraison est correcte sans y toucher. Changer la valeur, le jour où c'est utile, exige `docker compose up -d backend` (**recréation**), pas un `restart`. ⚠️ **Effet de bord** : plus de pause de quota = plus de repos entre scénarios → suspect n°1 devant un rouge **en campagne mais vert en isolation** (invariant #11). → piège #24.

📄 Les 25 pièges rencontrés, version longue et expliquée → **`docs/pieges.md`** (TOCTOU/verrous, ordre des verrous, tests de course **sans barrière ET sans balayage de décalage**, écritures par **cascade** hors inventaire, slots périmés, `.env`, WSL2, Drizzle…)
...
**Commentaires** — 🚨 **courts, sans emoji, pas de docblock de 20 lignes** (branche `simplify-match`, 15/08 : **−7 435 lignes sur 204 fichiers**, dont 171 en commentaires seuls). Un commentaire garde **la règle et le pointeur** ; l'histoire qui l'a produite vit dans `docs/`. C'est la règle du §✂️ plus bas, appliquée au code. **Ne jamais ré-enfler un docblock pédagogique.** ⚠️ Ceci **renverse** la ligne de `[FIX-LANDING-RAIL]` (« le commentaire EST la documentation »), qui avait servi à restaurer des blocs retirés — ne pas relire ce commit avec l'ancienne. → `docs/journal.md`, 15 août
  - ✅ **Backend dégraissé à son tour le 16/08** (2 526 → 1 910 lignes de commentaire, **−1 094 / +477**, 0 ligne de code touchée). Le format retenu : **2-3 phrases sur une fonction simple**, un peu plus sur une partie complexe, **plafond ~20 lignes d'affilée et 20 est acceptable** là où c'est vraiment tordu (7 blocs seulement passent 10 lignes). Ce qui saute : mesures chiffrées, codes de tickets, renvois aux tests hors dépôt, tableaux d'inventaire. Ce qui reste : le **pourquoi** d'un choix non évident, celui qu'on peut avoir à défendre à l'oral.
...
| `docs/modules.md`         | Les 14 modules (21 pts revendiqués, plafond 19), exigences PDF, candidats de réserve                                                        |
| `docs/stack.md`           | Versions des libs + arborescence réelle du repo                                                              |
| `docs/pieges.md`          | Les 25 pièges rencontrés, version longue                                                                     |
