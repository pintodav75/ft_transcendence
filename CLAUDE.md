...
- ⏳ **[FX-TABLE] EST FINI ET POUSSÉ, EN ATTENTE DE MERGE PAR DAVID** — branche `feature/fx-table-match-cards`, carte https://trello.com/c/xdrVnG3H en Review. Sous 640 px `MatchHistoryTable` rend des **cartes** au lieu de la table (les 3 écrans d'un coup : `/history`, page d'équipe, `/solo/$ladderId`) ; au-dessus, DOM **inchangé à la balise près** (vérifié nœud par nœud contre master). Campagne **20 scénarios / 349 checks / exit 0**. **2 commits, volontairement séparés** : le ticket, **et** `fix(audit)` sur `f-nav.mjs` — indépendant du ticket, à garder ou refuser librement. 🚨 **Ce 2ᵉ commit répare un exit 2 QUI TOMBE AUSSI SUR MASTER** : le panneau de résultats de la recherche recouvre les items du rail, et dès que la saisie « au » du §8 ramène assez de comptes (les `audit…` laissés par les campagnes précédentes **s'accumulent** et matchent tous — 13 en base au 31/07), le lien Solo devient intercepté → `locator.click` expire à 30 s, **campagne rouge sans un seul check rouge pour l'expliquer**. Durci le scénario plutôt que purgé la base (décision [FX-AUDIT]). ⚠️ **Le mécanisme reste amorcé ailleurs** : chaque campagne laisse des comptes derrière elle → carte à créer. ⚠️ **Rien n'a été vu à l'œil** (ni `coder-front` ni `reviewer-front` n'ouvrent un navigateur) : 3 points visuels à valider en capture avant merge, listés sur la carte. Ligne `docs/` à écrire **au merge**.
