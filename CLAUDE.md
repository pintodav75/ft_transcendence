...
- **Frontend** : F0, F0-A/B/C/D, FR1 Register, FR2 Login+2FA, F-Nav (teams + ranking), FL landing publique, **FT-1 + FT-1B** (`/teams`, mergés le 26/07 dans `640248b`) — **mergés**. **FT-2A** (`/teams/$teamId` en consultation) **mergé le 27/07** (commit `478c62b`, merge `9ef6406`). Restent à remplir : `/home`, `/games`, `/profile`, `/privacy`, `/terms`, et les 3 placeholders posés par FT-2A (`/players/$pseudo`, `/matches/$matchId`, `/ladders/$ladderId`). → `docs/frontend.md`
