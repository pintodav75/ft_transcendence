...
- 🎯 **POINT DE REPRISE — 1er août.** ✅ **LE RAIL SOCIAL EST TERMINÉ** (6 cartes FS-0→FS-5, + 2 correctifs), campagne **26 scénarios / 399 checks / exit 0**, console 0. Détail par carte → `docs/frontend.md`, récit daté → `docs/journal.md`. **Il n'y a PLUS de chantier en cours.** Prochaines cartes possibles : **[F-PLAYER]** (branche de William à 90 %) et **[F4]** (branche d'Adrien, renvoyée en correction) — les deux hors de master, voir plus bas. Ne restent ci-dessous que les conséquences des merges précédents qu'on doit connaître **sans avoir pensé à les demander** :
  ...
  **[F-PLAYER]** (https://trello.com/c/AZ3U4BZl, **branche de William à 90 %**) — voir la ligne `page/player` plus bas. 🚨 **PAS d'historique de matchs ni de comptes de jeu liés sur le profil d'un inconnu** — décision de David (vie privée) : ça se consulte sur **son propre** compte. **Ne pas le reproposer.** ⚠️ **Trois points à lui transmettre** : son écran de profil bloqué doit rester **identique dans les deux sens** (le serveur rend 404 aux deux, y compris au bloqueur) et ne proposer **aucune action** ; il doit **déclarer ce 404 dans son scénario d'audit** (`expectHttp`), sinon sa campagne sortira rouge ; et son bouton « Bloquer » fait **disparaître la page sous les pieds** de l'utilisateur — prévoir une redirection.
