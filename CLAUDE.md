...
- ⚠️ **MASTER A BOUGÉ PENDANT L'ABSENCE DE DAVID (6 → 10 août), du code, pas de la doc** : Walid a réduit la zone cliquable des liens de profil du rail ; **William a simplifié la `SearchBar` du rail, ajouté recherche d'adversaire + tri + pagination sur `/history` (front ET back, `order=asc|desc`), et RETIRÉ les devtools du routeur**. 4 commits, 31 fichiers, `openapi.yaml` + `api-types.gen.ts` régénérés (invariant #8 respecté). Conséquences ci-dessous (points 3 et 4, et le faux positif `axe` de [F4]).
...
- 🎯 **POINT DE REPRISE — 3 août, après `dbc1692`.** 🚨 **LE DÉVELOPPEMENT EST TERMINÉ.** Plus aucune carte en Todo, In Progress ou Review ; plus aucune branche de coéquipier hors de master ; le board est vide. **21 points revendiqués depuis le README, plafond 19.** **Il ne reste QUE le README et la préparation de la soutenance.** **Dans l'ordre :**
  ...
  3. ✅ **README RÉDIGÉ le 4 août** (anglais, 12 sections, 21 pts listés / plafond 19). ✅ **Bloc « Individual contributions » de David rempli le 13 août** (`fcd02a7`) — gabarit pour les 3 autres : ~15 lignes, 3 sous-parties, ton direct, **rien sur les tests**. ⚠️ **Restent les 3 blocs d'Adrien, Walid et William** — une section vide = README incomplet = motif de rejet. Enlève le motif de rejet n°1 **et** embarque les 2 modules gratuits (18 → 21 pts). 🚨 **Y mettre `seed:demo` + le tableau des comptes**, jamais `seed:dev`/`seed:social` (interne). 🚨 **Ne rien écrire sur les tests** : revendiquer des tests qu'on ne peut pas montrer est pire que le silence. 🚨 **Le bonus n'est évalué QUE si la partie obligatoire est parfaite** — un README incomplet n'enlève pas qu'une case, il annule les 4 points au-dessus de 14.
