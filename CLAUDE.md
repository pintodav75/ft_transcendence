...
- **Frontend** : **toutes les pages du rail sont remplies** — `/home`, `/teams`, `/teams/$teamId`, `/solo`, `/solo/$ladderId`, `/games`, `/games/$gameId`, `/ladders/$ladderId`, `/matches/$matchId`, `/matchmaking`, `/history`, `/disputes/$disputeId`. **Plus aucune page vierge, plus aucun cul-de-sac.** Le cycle challenge/accept est bouclable **de bout en bout à la souris** depuis F-MM. **Restent à remplir** : `/profile` (F4, Adrien), `/privacy`, `/terms`. → **`docs/frontend.md`**
...
- 🎯 **POINT DE REPRISE — 31/07 au soir.** **[F-DISPUTE] mergé** (commit `fbcc356`, merge `f6345c8`, doc `0bca44d`, aucune migration), campagne **19 scénarios / 329 checks / exit 0**. Deux cartes prêtes en Todo, aucune bloquée :
  1. **[F-ADMIN]** (https://trello.com/c/gkuGGusY, front + **1 ligne de back**) — la file d'arbitrage, invisible aujourd'hui. 🔴 **PRÉREQUIS : `isAdmin` n'est PAS exposé par l'API.** La colonne `users.is_admin` existe (`schema.ts:59`) et `frontend/src/types/auth.ts:11` la déclare **déjà**, mais ni `openapi.yaml` ni le handler `/users/me` ne la renvoient → elle vaut `undefined` en permanence. **Aucun contournement** : sonder `GET /disputes` produirait un 403 en console chez tous les non-admins. Ensuite : section « Disputed matches » sur `/home` (conditionnelle, une requête réservée aux admins) + les 3 issues de `POST /disputes/{id}/resolve` **sur la page de F-DISPUTE**, pas sur un 2ᵉ écran. ⚠️ Le scénario d'audit aura besoin d'un **compte admin**, absent des fixtures (le runner sait faire un `psql` depuis FT-4B). ⚠️ **Le lien d'en-tête du dossier de litige mène à `/matches/$matchId`, que `GET /matches/{id}` refuse en 403 à un admin non-participant** sur un match `disputed` : à conditionner dans ce ticket.
  2. **[F-PLAYER]** (https://trello.com/c/AZ3U4BZl, **branche de William déjà à 90 %**) — voir la ligne `page/player` plus bas. 🚨 **PAS d'historique de matchs ni de comptes de jeu liés sur le profil d'un inconnu** — décision de David (vie privée) : ça se consulte sur **son propre** compte. **Ne pas le reproposer.**
