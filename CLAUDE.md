...
- **Tests** : `npm run audit` (audit console Chrome) sort **0 sans filtre** depuis FT-2A — **19 scénarios, 329 checks, exit 0** (31/07). Vitest 27/27 (helpers purs) + **20 suites e2e Python** sur la vraie base de dev. → **`backend/tests/README.md`** et `frontend/tests/console-audit/README.md`

📄 Historique daté des merges, décisions et pièges de chaque ticket → **`docs/journal.md`**

### Prochaines actions

- 🎯 **POINT DE REPRISE — 31/07 au soir.** **[F-ADMIN] mergé** (aucune migration), campagne **20 scénarios / 344 checks / exit 0**. L'arbitrage est démontrable de bout en bout : onglet « Arbitration » conditionnel dans le rail (rouge, avec le compte des litiges ouverts), file `/admin/disputes`, et les 3 issues de `POST /disputes/{id}/resolve` sur la page du dossier. → `docs/journal.md`
...
- ✅ **[FX-TABLE] MERGÉ le 31/07** (commit `8fd6e85`, merge `66fc9e9`, **aucune migration**, front pur) — sous 640 px `MatchHistoryTable` rend des **cartes**, au-dessus le DOM est inchangé à la balise près ; sert `/history`, la page d'équipe et `/solo/$ladderId` d'un coup. ⚠️ **Mergé SANS validation visuelle** (3 points listés sur la carte, à regarder à l'occasion sur un vrai téléphone). ⚠️ **Le merge embarque un 2ᵉ commit hors ticket** (`dca2495`, `fix(audit)` sur `f-nav.mjs`) qui réparait un `exit 2` **qui tombait aussi sur master** : les comptes `audit…` laissés par chaque campagne **s'accumulent**, la recherche du §8 en ramenait assez pour que son panneau recouvre le lien Solo du rail. **Le mécanisme reste amorcé ailleurs** — carte de purge à créer. → `docs/journal.md`

- ✅ **`fix/team-409-messages` MERGÉ le 31/07** (**sans carte Trello**, décision de David — dette front de [BX-LEAVE] et [BX-DEL]) : les deux refus **409** d'équipe sont enfin traduits. ⚠️ **Les deux phrases ne disent PAS la même chose parce que les deux routes ne refusent pas sur les mêmes statuts** : retirer un membre refuse sur `LOCKING_STATUSES` (« ongoing match ») ; dissoudre refuse sur `ENGAGING_STATUSES`, **créneau ouvert inclus** (« **open or** ongoing match »). Ne pas uniformiser. `removeTeamMemberErrorMessage` prend un `intent: 'kick' | 'leave'` **obligatoire** (une phrase par sens — « ce joueur est engagé » est faux quand c'est moi qui pars) ; codes discriminés via `errorPayloadCode` (ex-`invitationErrorCode`, remonté) et **constantes typées contre la codegen**. ⚠️ **Seule la dissolution a un check d'audit** (M14 de `teams-matchmaking`, l'état bloquant y existait déjà) : le choix `kick`/`leave` est **sans filet automatisé**, monter une équipe adverse coûtait plus que le refus gardé.

- ✅ **[BX-LEAVE] MERGÉ le 31/07** (commit `46686fb`, merge `7081182`) — ⚠️ **migration `0024`** : `docker compose restart backend` au prochain pull. Quitter/être exclu d'une équipe **nettoie la composition** : match **actif** → **409 `engaged_in_match`** ; créneau **`pending`** → le départ passe et **le créneau est annulé** (refuser ferait un cul-de-sac : seul le capitaine peut annuler). Solde la dette « slots orphelins » de B5b. 🚨 **Le verrou de la route de départ ne suffit PAS** : `validateSide()` lit le roster **hors transaction**, donc `POST /matches` et `/accept` **revérifient la composition sous le même verrou** (`lineupOffRoster`) — mesuré : 3 créneaux fantômes sur 3 sans ça. ⚠️ **Seul le camp accepteur est revérifié** ; assouplir la route de départ, la dissolution ou la suppression de compte **rouvre le trou en silence**. ⚠️ **Le front n'explique pas le 409** (« Could not remove this player. ») → **carte de suivi à créer**. → `docs/journal.md`

- ✅ **[BX-HASPWD] MERGÉ le 31/07** (commit `fe14ece`, merge `ee710b8`, **aucune migration**) — le user authentifié porte désormais **`hasPassword`**, et le retrait de `passwordHash`/`totpSecret`, **recopié sur 8 sites**, est centralisé dans **`toAuthUser()` (`backend/src/utils/user.ts`) : seule fabrique autorisée**, toute colonne sensible ajoutée à `users` se traite là. 🚨 **`oauthProvider` NE DIT PAS si un compte a un mot de passe** : Google se rattache à un compte existant retrouvé par email **sans toucher au hash** → ce compte a les deux. **Ne jamais re-déduire l'un de l'autre.** ⚠️ Le profil public **ne renvoie pas** le champ (fuite sans usage). ⚠️ **Tranche front non faite** (le composant vit sur la branche [F4]) : **2 lignes à appliquer chez Adrien** — la garde de « Change password » se fonde sur `hasPassword === false`, plus sur `oauthProvider`, et son commentaire « KNOWN LIMIT » devient faux. → `docs/journal.md`
...
- 🚨 **UNE branche dort HORS de master** : **`feature/fs0-social-foundation`** (commit `3cb16ac`) — **[FS-0] rail social, codé par WALID**, carte https://trello.com/c/9Dz6Y3LN **en Review**. **Relue deux fois** (30/07 puis 31/07), les 3 bloquants sont corrigés ; **la balle est chez Walid** : rebase sur master + campagne complète verte (**19 scénarios** désormais). Sa branche est basée sur `25e9086`, donc sa campagne « 16 scénarios » **n'a jamais exécuté `matchmaking.mjs`**, qui bascule en **375 px** — précisément le viewport où FS-0 monte un `MobileHeader` neuf. Charger la mémoire `project_f_social_decomposition` avant d'y toucher ; review en local **contre `git merge-base`, jamais contre master**.
...
- 🚨 **LA BASE DE DEV CONTIENT DES DONNÉES DE DÉMO POSÉES À LA MAIN** (30/07, uuids déterministes `bbbb2222-…`, créneaux `pending` sur plusieurs ladders). **Ce n'est pas le seed. Ne pas les purger sans demander à l'équipe.** C'est l'audit qui a été durci ([FX-AUDIT]), pas la donnée qui a été supprimée — **un scénario ne doit jamais supposer qu'il est seul sur la base de dev**.
