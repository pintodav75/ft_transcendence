...
- 🔍 **AUDIT COMPLET CONTRE LE SUJET v21.1 — 1er août.** PDF lu **en entier**. Trois choses à savoir sans les demander : ⓵ 🚨 **le plafond de notation est 19 points** (chap. VII : le bonus s'arrête à 5 au-dessus de 14) — on est à 16, viser plus haut ne sert que de **marge** ; ⓶ **3 points sont gagnables sans une ligne de code applicatif** — *Custom design system* (`components/ui/` en compte **25**, le sujet en demande 10) et **Modules of choice / Major** pour le cycle compétitif (challenge/accept + Elo + disputes + jobs 24 h), qu'**aucun** module de la liste ne couvre ; les deux n'exigent qu'une **section de README** ; ⓷ **le README est le motif de rejet le plus net** — **en français** alors que le sujet impose l'anglais, sans sa 1ʳᵉ ligne en italique obligatoire, et privé de **toutes** les sections exigées. 📌 **Décision de David : README plus tard**, quand `/profile` sera là et les commits mieux répartis (Adrien et William ont travaillé avec lui, mais **David a tout committé** — le déséquilibre du `git log` est un artefact, à rétablir avant la soutenance). Détail → mémoires `project-audit-modules-v21` et `project-audit-gaps-obligatoires`.
  - ✅ **RESPONSIVE CORRIGÉ** — `[FIX-RESPONSIVE]`, branche `fix/responsive-mobile-nav`. Sous 1024 px il n'existait **ni navigation ni déconnexion** ; la nav vit désormais dans **`components/layout/PrimaryNav.tsx`**, servie par le rail **et** par un tiroir mobile. `f-nav` : 18 → **22 checks**, console 0. Pièges et vérifications → `docs/frontend.md`
  - ✅ **WCAG 2.1 AA — MIS EN CONFORMITÉ** (`[A11Y-AA]`, même branche) : `axe-core` **0 violation** sur 17 routes × 2 largeurs + 4 états interactifs, **et** la passe manuelle des critères qu'`axe` ne voit pas — c'est elle qui a trouvé les 4 défauts réels. 🚨 **2.4.2 « Page Titled » est de niveau A** et on l'échouait : 1 seul titre pour 13 routes → `lib/page-title.ts` (**table centrale, jamais un `useEffect` par page**). Plus : nouveau token **`--color-border-control`** (la bordure d'un champ n'est pas décorative — 1,44:1 → 3,50:1), bordure du bouton primaire à 3,34:1, et onglets resserrés pour tenir **320 px**. `border-subtle` reste sous 3:1 mais ne borde plus que de la décoration, **exemptée**. Détail et justifications → `docs/frontend.md`
