...
- **Frontend** : F0, F0-A/B/C/D, FR1 Register, FR2 Login+2FA, F-Nav (teams + ranking), FL landing publique, **FT-1 + FT-1B** (`/teams`, mergés le 26/07 dans `640248b`) — **mergés**. **FT-2A** (`/teams/$teamId` en consultation) **mergé le 27/07** (commit `478c62b`, merge `9ef6406`). **FT-2B** (onglet Manage capitaine : renommer, logo, ajouter, exclure, dissoudre + Leave team) — **livré le 27/07, review agent passée, audit 71/71** ; ⚠️ **commit et merge à enregistrer ici** une fois faits. Restent à remplir : `/home`, `/games`, `/profile`, `/privacy`, `/terms`, et les 3 placeholders posés par FT-2A (`/players/$pseudo`, `/matches/$matchId`, `/ladders/$ladderId`). → `docs/frontend.md`
- **Tests** : `npm run audit` (audit console Chrome) sort **0 sans filtre** depuis FT-2A — **7 scénarios, 71 checks** (FT-2B a ajouté `teams-manage`, 23 checks). Vitest 27/27 (helpers purs) + **19 suites e2e Python / 647 cas** (`cd backend/tests && python3 run_all.py`), sans mocks, sur la vraie base de dev. ⚠️ Les users de test sont **semés en SQL** avec un token forgé (la route `register` reste à 3/min, rien n'est désactivé) : `test_sentinel.py` garde ce couplage, `test_auth_contract.py` couvre la vraie route. → `backend/tests/README.md`
...
- **FT-2 a été découpé en trois le 27/07** (le ticket unique mélangeait lecture, 5 mutations et un composant de confirmation inexistant). Maquette de référence commune : **`vsmode-team-detail-demo.html`** à la racine — ⚠️ sa propre bande de stats y est illisible (son `.a-scrim` la recouvre), ce n'est pas un écart à corriger.
  ...
  - **PROCHAIN TICKET : [FT-2C]** https://trello.com/c/LnSfRghd — créer / annuler un match depuis la page équipe.
