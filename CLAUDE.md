...
### Infrastructure — I2 + I3 mergés (+ I4 en cours)

- 🚧 **I4 — origine unique HTTPS + proxy `/api`/`/media` + Docker fiable** (branche `feature/i4-infra-single-origin`, **non mergé**). Rend l'infra locale cohérente : le navigateur ne connaît **qu'une** origine `https://localhost:5173` (Vite HTTPS), qui proxifie `/api/*` → `https://backend:3000` (retrait du préfixe `/api`, `ws:true` pour `/api/ws/chat`, `secure:false` interne) et `/media/*` → `http://minio:9000` (retrait du préfixe `/media`, `changeOrigin` pour que les URLs présignées SigV4 restent valides). Plus de `localhost:3000`/`localhost:9000` côté front, plus de contenu mixte.
