...
// Self-contained "poster grid" of the caller's teams: owns the single GET /teams call and all
// three states (loading / error / empty), same shape as GamesCards for the game collection.
export function TeamsCards({ gameFilter }: TeamsCardsProps) {
  const { data, isLoading, isError } = useMyTeams();
  // Reference data, cached an hour under ['games'] — often already warm from the LadderSelect
  // filter above or the games page, so this rarely refetches.
  ...
  // GET /teams orders by name alone, which scatters two teams of the same game across the grid
  // — and since the tile *is* the game artwork, two identical posters far apart read as a
  // duplicate rather than as a pair.
  const ordered = [...shown].sort(
    (a, b) =>
      // Falls back to the id so the grouping still holds while ['games'] loads — the group
      // order is arbitrary for that frame, the grouping is not.
  ...
  return (
    ...
    // it is display:grid — restating it keeps the "list of N teams" announcement (same idiom as
    // GamesCards).
    <ul
      role="list"
