...
- ⚠️ **Le scénario laisse des comptes en base à chaque run** (`audit…@example.com`). Le motif a changé le 28/07 avec **[BX-DEL]** : ce n'est plus un **500** (la FK `match_participants.user_id` est passée de `restrict` à `cascade`), c'est un **409 assumé** — la route refuse tant que le compte est aligné dans un match **non terminé** (`engaged_in_match`) ou **capitaine d'une équipe** (`captain_of_team`). Le scénario ouvre des slots sans tous les refermer et crée une équipe. Pour redevenir auto-nettoyant il doit annuler ses slots **puis** dissoudre l'équipe — c'est le parcours de sortie imposé aux vrais utilisateurs, donc une couverture utile en soi.
