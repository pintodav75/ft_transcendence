...
## [F-PLAYER] — `/players/$pseudo`, la fiche joueur publique (mergée le 1er août)

Branche `feature/f-player-page` (William), **rebasée** puis squashée en un commit `4396106`, merge `430e908`. Signée `Omshinwa <fynmorph@gmail.com>`. **Aucune migration.** Back + front dans le même ticket.

**Ce que le back sert en plus.** `GET /users/{pseudo}` rendait `{ user }` ; il rend `{ user, friendship, rankings, teams }`, les trois lectures en parallèle.
- 🚨 **La projection de `user` devient EXPLICITE** (invariant #6). La version d'avant sélectionnait la ligne entière puis **retirait** cinq colonnes par déstructuration : `isAdmin` passait donc sans être déclaré au contrat. 🔑 **Une liste de ce qu'on RETIRE oublie par construction toute colonne ajoutée après elle** — c'est la leçon, pas le champ.
- `isAdmin` est maintenant **déclaré et servi volontairement** (pastille « Admin » sur la fiche). Ce n'est pas un secret : l'arbitrage relit `users.is_admin` en base à chaque appel. **Ne jamais élargir la projection par analogie** — `passwordHash` et `totpSecret` restent dehors.
- 🔑 **`rank` et `ladderSize` sont calculés avec l'`ORDER BY` EXACT de `GET /ladders/{id}/rankings`** (`elo desc, wins desc, losses asc, id asc`), départages compris, par fonction fenêtre. La fiche pointe vers ce classement d'un clic : deux tris diraient « #3 » ici et « #4 » là. ⚠️ Un `count(*) where elo > le mien` (l'implémentation naïve) **reste vert tant qu'aucun ex æquo n'existe** — le test force quatre lignes identiques en SQL et exige quatre rangs distincts.
- Sûreté de la fenêtre : un ladder `1v1` ne peut pas porter d'équipe (`teams.ts` refuse la création), donc partitionner sur `ladderId` ne mélange jamais joueurs et équipes.
- ⚠️ **`isCaptain` de `PlayerTeam` décrit le PROFIL CONSULTÉ, pas l'appelant** — l'inverse exact du champ homonyme de `TeamListItem`. Même forme, même nom, référent opposé : d'où un schéma distinct au contrat plutôt qu'une réutilisation.

**Ce que le front rend.** En-tête de dossier (silhouette de `TeamHero`, mais bandeau en **dégradé plat** : un joueur n'appartient à aucun jeu, donc pas de `GameBanner`), `<h1>` = nom d'affichage et `@pseudo` en sous-titre mono, pastilles Admin / Friends, « Membre depuis » / « Amis depuis », bio, **Rankings**, **Teams**. 🔑 **C'est la section Teams qui crée la navigation joueur → équipe** : le graphe ne courait que dans l'autre sens.

🚨 **NI HISTORIQUE DE MATCHS NI COMPTES DE JEU LIÉS** — décision produit du 31/07 (vie privée) : ça se lit sur **son propre** compte (`/history`, et `/profile` pour les comptes liés). Ne pas le reproposer.

⚠️ **Un `rankings` vide est l'état NORMAL d'un compte neuf** : une ligne naît du **premier résultat de match**, jamais d'une inscription (même asymétrie que `/solo`). La copie doit dire « rien joué », jamais « quelque chose manque ».

**Les trois correctifs de review** (appliqués avant merge, David) :
1. 🚨 **« Bloquer » n'armait rien sur la fiche d'un AMI.** Le bouton est offert dès `canInteract` (inconnu **ou** ami) mais la fenêtre de confirmation n'était montée que pour un inconnu : un clic, aucun message, aucune requête. Or **se brouiller avec quelqu'un qu'on connaît est la raison ordinaire de bloquer**. La porte du dialogue lit désormais `canInteract`.
2. 🚨 **Un blocage réussi laissait la fiche à l'écran avec « Ajouter en ami ».** Le serveur rend **404 au bloqueur aussi**, donc la page ne pouvait plus ni se rafraîchir ni proposer quoi que ce soit. **Choix retenu : la page se replie sur son écran de fin** (`PlayerErrorPanel`), pas une redirection — cette page **n'a pas de parent** (c'est la raison d'être de `BackButton`), un retour arrière pourrait ramener sur la liste que le joueur vient de quitter, et seul un écran peut **dire où défaire le blocage**. La région live reste montée à travers le basculement, sinon l'annonce disparaît avant d'être lue.
3. 🔑 **La fiche avait sa PROPRE copie des mutations de relation** (`player-mutations.ts`, 227 lignes), écrite avant le merge du rail social. Elles n'invalidaient **rien** du rail : bloquer depuis une fiche laissait la personne dans l'onglet Amis et hors de l'onglet des bloqués jusqu'au rechargement. La page appelle maintenant **`lib/friend-mutations.ts`**, qui sait quelle liste chaque acte déplace, sépare « retirer un ami » de « annuler ma demande » (même route, listes différentes) et resynchronise la présence sur une acceptation automatique. **`player-mutations.ts` ne garde que les MOTS** — les phrases du rail nomment la personne parce qu'il en liste dix, ici toute la page parle déjà d'une seule.

⚠️ **La branche était basée sur master AVANT le rail** (FS-0 seul) : rebase obligatoire, sans quoi `friend-mutations.ts` n'existe pas.

⚠️ **Deux rouges dans SA PROPRE suite backend**, tous deux des listes de champs attendus restées en arrière du contrat (`isAdmin` sur `PublicUser`, `elo` sur `PlayerTeam`, les deux `required` dans `openapi.yaml` et affichés par la page). Le code avait raison, le test avait vieilli. **27/27 après correction.**
