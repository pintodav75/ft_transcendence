...
**22 suites** (`test_teams_leave.py` ajoutée le 31/07 par BX-LEAVE ; `test_matches_open_slots.py` ajoutée le 30/07 par B-MM ; les 9 suites matchmaking rejouées vertes ce jour-là, run complet des autres du 27/07). Aucune dépendance à installer : uniquement la
stdlib Python 3.
...
| `test_teams_leave.py` | BX-LEAVE | `DELETE /teams/:id/members/:userId` : un départ ne laisse plus de joueur **aligné** derrière lui. Les **deux chemins** de la route (départ volontaire **et** exclusion par le capitaine) sur les **deux** états de match : un **match actif** rend **409 `engaged_in_match`** (même code que BX-DEL) sans rien écrire, un **créneau `pending`** laisse partir mais tombe en `cancelled` avec la composition du partant nettoyée. 🔑 **Le check central n'est pas un code HTTP** : après le départ, le nombre de camps du match alignant le partant doit être **0** — avant le correctif il montait à **2**, l'ex-coéquipier fondant une équipe sur le même ladder et **acceptant le créneau de son ancienne équipe** (la garde anti-auto-acceptation compare les `team_id` des deux camps, pas les joueurs). Vérifie la règle de destinataires **en négatif** (invariant #2) : sur une exclusion, seul le reste de la composition est notifié — ni l'**acteur** (le capitaine), ni le **partant** —, et alice, à la fois capitaine et alignée, reçoit **une seule** notif (dédoublonnage de `notify()`). Contrôles **positifs** obligatoires : le **banc** part sans rien casser, un membre non aligné part malgré un match actif de son équipe, un match **`completed`** ne bloque plus **et garde sa composition intacte** (on ne réécrit pas l'histoire), un joueur aligné sur **plusieurs** créneaux les emporte **tous et eux seuls**. Couvre enfin l'ordre de décision choisi : l'**idempotence prime sur la garde** — retirer un **non-membre** rend 200 même s'il traîne encore dans un match actif (l'état hérité que le ticket corrige), jamais 409 |
...
| `test_users_profile.py` | F-PLAYER | `GET /users/{pseudo}` enrichi de **`rankings`** et **`teams`**. Projection EXACTE de `PublicUser` (⚠️ `isAdmin` fuyait tant que le handler retirait des colonnes au lieu d'en choisir). État vide d'un compte neuf (`[]` des deux côtés — une ligne de classement naît du **premier résultat de match**, jamais d'une inscription). 🔑 Surtout : **le rang est celui de `GET /ladders/{id}/rankings`, départages compris** — quatre joueurs forcés en SQL à des lignes IDENTIQUES doivent recevoir quatre rangs DISTINCTS, ce qu'un `count(*) where elo > le mien` (l'implémentation naïve) rate en restant vert sur des Elo différents. Tri des lignes par ladder joué le plus récemment. Côté `teams` : tri par nom et surtout **`isCaptain` qui décrit le PROFIL CONSULTÉ, pas l'appelant** — le même profil lu par deux comptes doit rendre la même couronne (c'est le check qui attrape une confusion avec `TeamListItem`) |
